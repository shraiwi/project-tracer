...
int64_t flasher_max_bit_period;     // the maximum bit period
...
void flasher_config(uint16_t min_range, int64_t min_bit_period, int64_t max_bit_period, int64_t bin_size, int64_t sample_period, int64_t epoch) {
    ...
    flasher_max_bit_period = max_bit_period;
...
int8_t flasher_feed(uint16_t adc_sample, int64_t epoch) {
    ...
    uint16_t adc_buf_min = UINT16_MAX, adc_buf_max = 0, adc_buf_range = 0;
    ...
    if (adc_buf_range < flasher_min_range) return -1;
    ...
    if (flasher_last_value != current_value) {
        ...
        if (dt > flasher_min_bit_period && dt < flasher_max_bit_period) {
            out_bit = (dt + (flasher_bin_size >> 1)) / flasher_bin_size;
            flasher_last_bit_flip = epoch;
        }
    }
